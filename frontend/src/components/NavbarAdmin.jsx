  ...
  return (
    <div className={`bg-white shadow-md h-screen flex flex-col transition-all duration-500 ease-in-out ${isExpanded ? 'w-60' : 'w-20'}`}>
      ...
      <nav className="flex-1 mt-4 space-y-2">
        ...
        {/* Logout Button */}
        <LogoutButton className={`flex items-center gap-3 px-4 py-2 rounded-lg my-4 mx-16 transition-all duration-300`}/>
      </nav>
      ...
      <div className="mb-6">
        <Link
          to="/profiladmin"
          className={`flex items-center gap-3 px-4 py-2 rounded-lg mx-2 transition-all duration-300 ${location.pathname === '/profiladmin' ? 'bg-blue-200 text-blue-700 font-semibold' : 'text-gray-600 hover:bg-blue-100'}`}
        >
          <User size={22} />

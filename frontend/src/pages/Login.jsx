...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage(data.error || 'Login gagal. Periksa kembali email dan password.');
        return;
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-8 w-96">
        ...
        <input type="text" name="email" placeholder="Email" className="w-full p-2 mb-3 border rounded" onChange={handleChange} required />
